...
public class SoftShrinkNode extends UnaryNode {
	public float alpha;
	public SoftShrinkNode(Node left) {
		super(left);
		alpha = 0.5f;
	}
	public SoftShrinkNode(float alpha, Node left) {
		super(left);
		this.alpha = alpha;
	}
	public INDArray child_evaluate() {
		...
		for(int i = 0; i < m.length(); i++) {
			m.putScalar(i, Functions.softshrink(alpha, m.getFloat(i)));
		}
		return this.m;
	}
