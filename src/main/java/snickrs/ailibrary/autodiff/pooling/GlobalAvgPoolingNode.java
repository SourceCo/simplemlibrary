...
public class GlobalAvgPoolingNode extends UnaryNode {
	public boolean keepdims;
	public int[] axes;
	public long n;
	public GlobalAvgPoolingNode(Node left, boolean keepdims) {
		super(left);
		this.keepdims = keepdims;
		this.n = 1;
		long[] shape = left.shape();
		this.axes = range(2, shape.length);
		for(int i = 0; i < axes.length; i++) {
			this.n *= shape[axes[i]];
		}
	}
	public GlobalAvgPoolingNode(Node left) {
		super(left);
		this.keepdims = false;
		this.n = 1;
		long[] shape = left.shape();
	...
	public void child_diff(INDArray upstream) {
		...
		partials.set(0, keepdims ? upstream.broadcast(children.get(0).m.shape()).div(n) : upstream.reshape(new_shape).broadcast(children.get(0).m.shape()).div(n));
	}
