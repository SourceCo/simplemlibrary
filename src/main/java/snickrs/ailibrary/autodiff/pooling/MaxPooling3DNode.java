...
public class MaxPooling3DNode extends UnaryNode {
	public INDArray mask;
	public Pooling3DConfig config;
	public MaxPooling3DNode(Node left, Pooling3DConfig config) {
		super(left);
		this.config = config;
	}
	public MaxPooling3DNode(Node left) {
		super(left);
		...
		config.setPH(0);
		config.setPW(0);
		config.setType(Pooling3DType.MAX);
	}
	public void recalculate_dims() {
		
	}
	public INDArray child_evaluate() {
		this.m = Nd4j.cnn().maxPooling3d(children.get(0).evaluate(), config);
		return this.m;
	}
            	...
            	for(int d = 0; d < shape[2]; d++) {
            		for (int h = 0; h < shape[3]; h++) {
            			for (int w = 0; w < shape[4]; w++) {
            				...
            				double max = this.m.getDouble(n, c, d, h, w);
            				this.mask.put(new INDArrayIndex[] {
                                NDArrayIndex.point(n), NDArrayIndex.point(c), NDArrayIndex.interval(dStart, dEnd), 
                                NDArrayIndex.interval(hStart, hEnd), NDArrayIndex.interval(wStart, wEnd)
            				}, region.eq(max).castTo(Nd4j.defaultFloatingPointType()).mul(upstream.getDouble(n, c, d, h, w)));
            			}
            		}
            	}
            }
