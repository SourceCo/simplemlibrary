...
public class MaxPooling1DNode extends UnaryNode {
	public INDArray mask;
	public Pooling2DConfig config;
	public MaxPooling1DNode(Node left, long k, long s, long p, long d, PaddingMode mode) {
		super(left);
		...
		config.setPH(0);
		config.setPW(p);
		config.setType(Pooling2DType.MAX);
		config.setPaddingMode(mode);
	}
	public MaxPooling1DNode(Node left) {
		super(left);
		this.config = new Pooling2DConfig();
		config.setKH(1);
		config.setKW(2);
	...
	public INDArray child_evaluate() {
		...
		this.m = Nd4j.squeeze(Nd4j.cnn().maxPooling2d(children.get(0).m.reshape(shape[0], shape[1], 1, shape[2]), config), 2);
		return this.m;
	}
        ...
        for (int n = 0; n < shape[0]; n++) {
            for (int c = 0; c < shape[1]; c++) {
                    for (int w = 0; w < shape[2]; w++) {
                        ...
                        INDArray region = children.get(0).m.get(
                                NDArrayIndex.point(n), NDArrayIndex.point(c), NDArrayIndex.interval(wStart, wEnd)
                        );
                        double max = this.m.getDouble(n, c, w);
                        this.mask.put(new INDArrayIndex[] {
                                NDArrayIndex.point(n), NDArrayIndex.point(c), NDArrayIndex.interval(wStart, wEnd)
                        }, region.eq(max).castTo(Nd4j.defaultFloatingPointType()).mul(upstream.getDouble(n, c, w)));
                    }
            }
        }
