...
public class MaxPooling2DNode extends UnaryNode {
	public INDArray mask;
	public Pooling2DConfig config;
	public MaxPooling2DNode(Node left, Pooling2DConfig config) {
		super(left);
		this.config = config;
	}
	public MaxPooling2DNode(Node left) {
		super(left);
	...
	public INDArray child_evaluate() {
		this.m = Nd4j.cnn().maxPooling2d(children.get(0).evaluate(), config);
		return this.m;
	}
            ...
            for (int c = 0; c < shape[1]; c++) {
                for (int h = 0; h < shape[2]; h++) {
                    for (int w = 0; w < shape[3]; w++) {
                        ...
                        double max = this.m.getDouble(n, c, h, w);
                        this.mask.put(new INDArrayIndex[] {
                                NDArrayIndex.point(n), NDArrayIndex.point(c),
                                NDArrayIndex.interval(hStart, hEnd), NDArrayIndex.interval(wStart, wEnd)
                        }, region.eq(max).castTo(Nd4j.defaultFloatingPointType()).mul(upstream.getDouble(n, c, h, w)));
                    }
                }
            }
        }
